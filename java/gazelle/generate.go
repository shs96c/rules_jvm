...
import (
	...
	localproto "github.com/bazel-contrib/rules_jvm/java/gazelle/private/proto"
...
func generateProtoLibraries(l *javaLang, args language.GenerateArgs, log zerolog.Logger, res *language.GenerateResult, cfg *javaconfig.Config) {
	...
	// Also process existing proto_library rules from the BUILD file that weren't
	// regenerated by the proto extension. This ensures java_library rules wrapping
	// existing proto libraries get their _java_packages attribute set for resolution.
	if args.File != nil {
		processExistingProtoLibraries(l, args, log, res, protoRuleNames)
	}
...
// processExistingProtoLibraries handles proto_library rules that already exist in the BUILD file
// but weren't regenerated by the proto extension. For each such rule, it parses the .proto files
// to extract java_package options and sets the _java_packages attribute on corresponding
// java_library rules so they can be resolved as dependencies.
func processExistingProtoLibraries(l *javaLang, args language.GenerateArgs, log zerolog.Logger, res *language.GenerateResult, alreadyProcessed []string) {
	// Build a set of already-processed proto rule names for quick lookup
	processedSet := make(map[string]bool)
	for _, name := range alreadyProcessed {
		processedSet[name] = true
	}

	// Build a map of existing java_library rules by name for quick lookup
	existingJavaLibraries := make(map[string]*rule.Rule)
	for _, r := range args.File.Rules {
		if r.Kind() == "java_library" {
			existingJavaLibraries[r.Name()] = r
		}
	}

	// Process existing proto_library rules
	for _, r := range args.File.Rules {
		if r.Kind() != "proto_library" {
			continue
		}
		if processedSet[r.Name()] {
			// Already processed via args.OtherGen
			continue
		}

		// Get the srcs attribute to find .proto files
		srcs := r.AttrStrings("srcs")
		if len(srcs) == 0 {
			continue
		}

		// Parse proto files to extract java_package and other info
		var javaPackage string
		var outerClassName string
		var messages, enums, services []string

		for _, src := range srcs {
			protoPath := path.Join(args.Dir, src)
			parsed, err := localproto.ParseFile(protoPath)
			if err != nil {
				log.Debug().Err(err).Str("proto", protoPath).Msg("failed to parse existing proto file")
				continue
			}

			if jp, ok := parsed.Options["java_package"]; ok && javaPackage == "" {
				javaPackage = jp
			}
			if oc, ok := parsed.Options["java_outer_classname"]; ok && outerClassName == "" {
				outerClassName = oc
			...
			services = append(services, parsed.Services...)
		}
		...
		// Find the corresponding java_library rule (follows naming convention: foo_proto -> foo_java_library)
		jlName := strings.TrimSuffix(r.Name(), "_proto") + "_java_library"
		existingJL, exists := existingJavaLibraries[jlName]
		if !exists {
			log.Debug().Str("proto_rule", r.Name()).Str("expected_java_library", jlName).Msg("no matching java_library found for existing proto_library")
			continue
		}

		packageName := types.NewPackageName(javaPackage)
		log.Debug().Str("pkg", packageName.Name).Str("rule", jlName).Msg("setting java package for existing proto java_library")

		// Set the _java_packages attribute on the existing java_library
		existingJL.SetPrivateAttr(packagesKey, []types.ResolvableJavaPackage{*types.NewResolvableJavaPackage(packageName, false, false)})

		// Build class list for class-level resolution
		var protoClasses []types.ClassName
		if outerClassName != "" {
			protoClasses = append(protoClasses, types.NewClassName(packageName, outerClassName))
		}
		...
		if len(protoClasses) > 0 {
			existingJL.SetPrivateAttr(classesKey, protoClasses)
			ruleLabel := label.New("", args.Rel, jlName)
			l.classExportCache[ruleLabel.String()] = classExportInfo{
				classes:  protoClasses,
				testonly: false,
			...
			log.Debug().
				...
				Str("label", ruleLabel.String()).
				Strs("classes", classNames).
				Msg("registered existing proto classes for class-level resolution")
		}

		// Add the rule to res.Gen so that it gets indexed via Imports()
		res.Gen = append(res.Gen, existingJL)
		res.Imports = append(res.Imports, types.ResolveInput{
			PackageNames: sorted_set.NewSortedSetFn([]types.PackageName{packageName}, types.PackageNameLess),
		})
	}
