...
func (jr *Resolver) Resolve(c *config.Config, ix *resolve.RuleIndex, rc *repo.RemoteCache, r *rule.Rule, imports interface{}, from label.Label) {
	...
	jr.populateAttr(c, packageConfig, r, "deps", resolveInput.ImportedPackageNames, resolveInput.ImportedClasses, ix, isTestRule, from, resolveInput.PackageNames, resolveInput.ExistingDeps)
	jr.populateAttr(c, packageConfig, r, "exports", resolveInput.ExportedPackageNames, resolveInput.ExportedClassNames, ix, isTestRule, from, resolveInput.PackageNames, resolveInput.ExistingExports)
...
func (jr *Resolver) populateAssociatesAttr(c *config.Config, ix *resolve.RuleIndex, resolveInput types.ResolveInput, r *rule.Rule, isTestRule bool, from label.Label) {
	...
	for _, pkg := range resolveInput.PackageNames.SortedSlice() {
		for _, mainLabel := range jr.productionAssociateLabels(c, ix, pkg, from).SortedSlice() {
...
// productionAssociateLabels returns the production library that uniquely
// provides pkg. A package with several providers yields nothing: associate
// compilation joins the test into the production Kotlin module (internal
// visibility, smart-cast stability), which is only safe when the package
// unambiguously belongs to one library.
func (jr *Resolver) productionAssociateLabels(c *config.Config, ix *resolve.RuleIndex, pkg types.PackageName, from label.Label) *sorted_set.SortedSet[label.Label] {
...
func (jr *Resolver) populateAttr(c *config.Config, pc *javaconfig.Config, r *rule.Rule, attrName string, requiredPackageNames *sorted_set.SortedSet[types.PackageName], importedClasses *sorted_set.SortedSet[types.ClassName], ix *resolve.RuleIndex, isTestRule bool, from label.Label, ownPackageNames *sorted_set.SortedSet[types.PackageName], existingLabels *sorted_set.SortedSet[label.Label]) {
	...
	if existingLabels != nil {
		for _, l := range existingLabels.SortedSlice() {
			preferredExistingLabels[normalizeLabelPreference(l, c.RepoName, from)] = struct{}{}
		}
	}
	...
	for _, imp := range requiredPackageNames.SortedSlice() {
		...
		dep, ambiguous := jr.resolveSinglePackageWithAmbiguity(c, pc, imp, ix, from, isTestRule, ownPackageNames, pkgClasses, preferredExistingLabels, ruleHasKotlinSources(r))
		if dep != label.NoLabel {
			...
			for _, className := range classesByPackage[imp] {
				...
				if l != label.NoLabel {
					if jr.shouldUseMavenClassLabel(c, l, dep, className, ix, isTestRule, false) {
						labels.Add(simplifyLabel(c.RepoName, l, from))
						continue
					}
					l = label.NoLabel
				}
				if l := jr.resolveClassFromCrossResolver(c, pc, className, ix, from, preferredExistingLabels); l != label.NoLabel {
					labels.Add(l)
					continue
				}
				if isTestRule {
				...
				if jr.ruleDeclaresAnyClassInPackage(dep, className.PackageName()) {
					...
					// The Maven claim here is package-level only. A lowercase symbol is a
					// Kotlin top-level function reference, which class metadata cannot
					// declare, so only class-cased symbols may leave the workspace owner.
					if l != label.NoLabel && symbolLooksLikeClass(className) && jr.shouldUseMavenClassLabel(c, l, dep, className, ix, isTestRule, false) {
						labels.Add(simplifyLabel(c.RepoName, l, from))
						continue
					}
				}
		...
		if ambiguous && len(classesByPackage[imp]) > 0 {
			...
			if !resolvedAny {
				jr.lang.logger.Error().
					...
					Strs("classes", pkgClasses).
					Stringer("from", from).
					Msg("unable to resolve package or any of its classes")
	...
	if importedClasses != nil && ownPackageNames != nil {
		kotlinOwnPackageGuess := ruleHasKotlinSources(r)
		for _, className := range importedClasses.SortedSlice() {
			if !ownPackageNames.Contains(className.PackageName()) {
				continue
			}
			// A class this rule itself declares (e.g. in a module sub-package) needs
			// no dependency; even an explicit directive must not force a self-dep
			// through a published copy of the workspace.
			...
			if l != label.NoLabel {
				// A rule named after the artifact's directory (e.g. an SCC group for
				// org/apache/avro) makes the raw name heuristic misread the real Maven
				// owner as a published self-copy; gate on workspace declarations instead.
				if jr.shouldUseMavenClassLabel(c, l, from, className, ix, isTestRule, kotlinOwnPackageGuess) {
					labels.Add(simplifyLabel(c.RepoName, l, from))
					continue
				}
				l = label.NoLabel
			...
			if l != label.NoLabel && jr.shouldUseMavenClassLabel(c, l, from, className, ix, isTestRule, kotlinOwnPackageGuess) {
				labels.Add(simplifyLabel(c.RepoName, l, from))
			}
		}
	}
...
// symbolLooksLikeClass reports whether an imported symbol follows the class
// naming convention, mirroring ParseClassName's package/class split.
...
// preferredExistingMavenPackageOwner returns the existing dependency that Maven's
// package index lists as an owner of pkg, provided exactly one existing
// dependency qualifies.
func (jr *Resolver) preferredExistingMavenPackageOwner(c *config.Config, pc *javaconfig.Config, pkg types.PackageName, from label.Label, preferences map[label.Label]struct{}) label.Label {
	if len(preferences) == 0 {
		return label.NoLabel
	}
	var candidates []label.Label
	l, err := jr.lang.mavenResolver.Resolve(pkg, pc.ExcludedArtifacts(), pc.MavenRepositoryName())
	if err == nil && l != label.NoLabel {
		candidates = append(candidates, l)
	}
	var multipleExternal *maven.MultipleExternalImportsError
	if errors.As(err, &multipleExternal) {
		for _, possible := range multipleExternal.PossiblePackages {
			pl, parseErr := label.Parse(possible)
			if parseErr != nil {
				continue
			}
	...
	return preferredExistingClassCandidate(candidates, preferences, c.RepoName, from)
}
...
func (jr *Resolver) resolveSinglePackageWithAmbiguity(c *config.Config, pc *javaconfig.Config, imp types.PackageName, ix *resolve.RuleIndex, from label.Label, isTestRule bool, ownPackageNames *sorted_set.SortedSet[types.PackageName], pkgClasses []string, preferredExistingLabels map[label.Label]struct{}, allowMavenCompileGroup bool) (out label.Label, ambiguous bool) {
	...
	if len(matches) == 1 {
		l := matches[0].Label
		// An existing dependency that Maven indexes as an owner of the package is
		// kept over an external candidate the rule has never compiled against:
		// index lookups run before the Maven resolver, so without this check a
		// generated-code plugin's package claim silently replaces the checked-in
		// Maven dependency. In-repo providers still win so code moving into the
		// workspace repoints as usual, and explicit resolve directives return
		// before this check.
		if l.Repo != "" && l.Repo != c.RepoName {
			simplified := simplifyLabel(c.RepoName, l, from)
			if _, ok := preferredExistingLabels[normalizeLabelPreference(simplified, c.RepoName, from)]; !ok {
				if existing := jr.preferredExistingMavenPackageOwner(c, pc, imp, from, preferredExistingLabels); existing != label.NoLabel {
...
func (jr *Resolver) resolveSinglePackage(c *config.Config, pc *javaconfig.Config, imp types.PackageName, ix *resolve.RuleIndex, from label.Label, isTestRule bool, ownPackageNames *sorted_set.SortedSet[types.PackageName], pkgClasses []string) (out label.Label) {
	out, _ = jr.resolveSinglePackageWithAmbiguity(c, pc, imp, ix, from, isTestRule, ownPackageNames, pkgClasses, nil, false)
	return out
}
...
func (jr *Resolver) resolveSingleClass(c *config.Config, pc *javaconfig.Config, className types.ClassName, ix *resolve.RuleIndex, from label.Label, isTestRule bool, preferredExistingLabels map[label.Label]struct{}) (out label.Label) {
	...
	if len(candidates) == 0 {
		...
		return jr.resolveClassFromCrossResolver(c, pc, className, ix, from, preferredExistingLabels)
	}
...
func (jr *Resolver) shouldUseMavenClassLabel(c *config.Config, mavenLabel label.Label, owner label.Label, className types.ClassName, ix *resolve.RuleIndex, isTestRule bool, kotlinOwnPackageGuess bool) bool {
	if !mavenLabelLooksLikeWorkspaceOwner(mavenLabel, owner) {
		return true
	}

	if jr.workspaceDeclaresClass(c, className, ix, isTestRule) {
		return false
	}
	if kotlinOwnPackageGuess {
		// Kotlin refers to own-package names without imports (default imports,
		// file-private types, inherited scope), so an undeclared own-package
		// class from Kotlin sources is too weak a signal to pull in a published
		// copy of the workspace. Java compilation requires such a name to be a
		// real same-package class, so only Java sources may use the Maven label.
		return false
	}
...
func (jr *Resolver) workspaceDeclaresClass(c *config.Config, className types.ClassName, ix *resolve.RuleIndex, isTestRule bool) bool {
	return packageClassIndexDeclaresClass(jr.buildPackageClassIndex(c, className.PackageName(), ix), className, isTestRule)
}

func packageClassIndexDeclaresClass(pci *packageClassIndex, className types.ClassName, isTestRule bool) bool {
	bareClassName := className.BareOuterClassName()
	if len(pci.prod[bareClassName]) > 0 {
		return true
	}
	return isTestRule && len(pci.test[bareClassName]) > 0
}
...
// target owns the enclosing package (a split package). An existing dependency that
// Maven indexes as an owner of the class's package is kept over a cross-resolver
// candidate the rule has never compiled against: swapping in a generated-code copy
// would duplicate the package's classes across two classpath jars.
func (jr *Resolver) resolveClassFromCrossResolver(c *config.Config, pc *javaconfig.Config, className types.ClassName, ix *resolve.RuleIndex, from label.Label, preferredExistingLabels map[label.Label]struct{}) label.Label {
	...
	if candidates.Len() == 1 {
		l := simplifyLabel(c.RepoName, candidates.SortedSlice()[0], from)
		if _, ok := preferredExistingLabels[normalizeLabelPreference(l, c.RepoName, from)]; ok {
			return l
		}
		if existing := jr.preferredExistingMavenPackageOwner(c, pc, className.PackageName(), from, preferredExistingLabels); existing != label.NoLabel {
			return simplifyLabel(c.RepoName, existing, from)
		}
		return l
	}
