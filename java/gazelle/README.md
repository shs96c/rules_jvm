...
## Resolving classes provided by other Gazelle extensions

Some Java classes are generated by other Gazelle extensions rather than by this
one — for example a Wire or third-party protobuf extension that emits Java/Kotlin
from `.proto` files. These often produce a *split package*: some classes of a Java
package come from the external extension's target, while hand-written sources in
the same package are built by a `java_library`/`kt_jvm_library` this extension
generates.

An external extension can contribute these resolutions at **class granularity** by
implementing Gazelle's [`resolve.CrossResolver`][crossresolver] interface and
answering fully-qualified class-name imports for the `java` language:
...
func (e *myExtension) CrossResolve(c *config.Config, ix *resolve.RuleIndex, imp resolve.ImportSpec, lang string) []resolve.FindResult {
    ...
    // imp.Imp is a dotted fully-qualified class name, e.g. "com.example.foo.MyMessage"
    // (inner classes included, e.g. "com.example.foo.Outer.Inner").
    if target, ok := e.classToTarget[imp.Imp]; ok {
...
Register the extension in the same Gazelle binary as this one. Because this
extension intentionally keeps classes out of Gazelle's global rule index, every
class-level lookup falls through to registered `CrossResolver`s — so an external
provider is consulted even when an in-repo `java_library` already owns the
enclosing package. A `# gazelle:resolve` directive still takes precedence, and an
in-repo target that declares the class wins over a cross-resolver.
